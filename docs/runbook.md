...
`alembic upgrade head` đã được xác minh từ migration 0001 đến 0008 trên PostgreSQL 18.3 cô lập, có pgvector 0.8.2; chạy lại `upgrade head` không thay đổi schema. Migration 0008 thêm `campaigns.content_plan_json` kiểu JSON NOT NULL default `{}`. Migration environment tự tạo/nâng `alembic_version.version_num` lên `VARCHAR(128)` trên PostgreSQL vì revision IDs của repo dài hơn giới hạn mặc định 32 ký tự. Bằng chứng này chỉ xác nhận migrations và extension trên database test; chưa xác nhận toàn bộ API/worker/Redis/MinIO stack với PostgreSQL.
