...
| 1. Môi trường chạy | Python/Node deps; PostgreSQL/Redis; object storage hoặc local test adapter | Cài từ lock/manifest; migration 0001→0007 trên PostgreSQL/pgvector cô lập đã pass; còn nghiệm thu API, worker, scheduler, Redis/MinIO và health/readiness trên full runtime. Không dùng DB production. |
...
| 3. Campaign/content/approval/export | Phase 2; DB schema/API/OpenAPI | Campaign/brief create/edit, strategy/content slots; idempotent generate và revise jobs; confirmed profile + relevant context; citations; immutable draft/version; scope-limited AI revise với optimistic concurrency; human approval; CSV/XLSX download. Brief edit đã có; strategy/slots và media asset attachment còn trong PLAN-001/MEDIA-001. Fixture integration passes; live DeepSeek path remains unverified. Publish hash guard còn thiếu. |
...
Chi tiết theo ID và tiêu chí ở [task-board.md](task-board.md). Campaign brief edit có API/UI và version conflict guard. Strategy/content slots (PLAN-001) và image upload/attachment (MEDIA-001) vẫn chưa hoàn tất. AI revise code path đã có API/worker fixture test và mock browser coverage; còn provision DeepSeek key + live model/generation/revise smoke → full PostgreSQL/Redis/MinIO runtime → real-provider browser E2E → Meta permissions → hardening. Manual metrics import, report, feedback, brief revision và outcome comparison đã có fixture/API path; không coi đó là Meta sync, live provider test hoặc production runtime.
