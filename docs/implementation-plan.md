...
- Chủ dự án đã chấp thuận gửi đoạn trích tài liệu và Brand Profile tới DeepSeek. Upload-driven profile worker dùng configured DeepSeek adapter; content API tạo durable/idempotent job và worker lưu draft có citations sau tenant/revision/relevance checks. Fixture tests đã xác minh; live key/model smoke chưa chạy.
...
| 3. Campaign/content/approval/export | Phase 2; DB schema/API/OpenAPI | Campaign/brief thật; idempotent generate job; confirmed profile + relevant context; citations; immutable draft/version; human approval; CSV/XLSX download. Fixture integration passes; live DeepSeek path remains unverified. Publish hash guard còn thiếu. |
...
Chi tiết theo ID và tiêu chí ở [task-board.md](task-board.md). Thứ tự còn lại: provision DeepSeek key + live model/generation smoke → isolated PostgreSQL/Compose runtime → browser E2E cho AI flow → Meta permissions → hardening. Manual metrics import, report, feedback, brief revision và outcome comparison đã có fixture/API path; không coi đó là Meta sync, live provider test hoặc production runtime.
