...
- Python 3.11.16 và Node 26.7.0 có sẵn. Redis local trả `PONG`; Docker/Podman và MinIO không khả dụng. Đã tạo disposable PostgreSQL 18.3 + pgvector 0.8.2 riêng trong `/private/tmp` và xác minh migration 0001→0008 + rerun; PostgreSQL 15 trên máy không bị thay đổi. Python và npm dependencies đã được cài trong worktree.
...
| 1. Môi trường chạy | Python/Node deps; PostgreSQL/Redis; object storage hoặc local test adapter | Cài từ lock/manifest; migration 0001→0008 trên PostgreSQL/pgvector cô lập đã pass; còn nghiệm thu API, worker, scheduler, Redis/MinIO và health/readiness trên full runtime. Không dùng DB production. |
...
| 3. Campaign/content/approval/export | Phase 2; DB schema/API/OpenAPI | Campaign/brief create/edit, strategy/content slots; idempotent generate và revise jobs; confirmed profile + relevant context; citations; immutable draft/version; scope-limited AI revise với optimistic concurrency; human approval; CSV/XLSX download. PLAN-001 strategy/slot generation đã có migration/API/worker/UI và fixture + browser tests; MEDIA-001 image asset attachment remains open. Live DeepSeek path remains unverified. Publish hash guard còn thiếu. |
