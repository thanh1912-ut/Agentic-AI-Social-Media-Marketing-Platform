...
| 3. Campaign/content/approval/export | Phase 2; DB schema/API/OpenAPI | Campaign/brief thật; idempotent generate và revise jobs; confirmed profile + relevant context; citations; immutable draft/version; scope-limited AI revise với optimistic concurrency; human approval; CSV/XLSX download. Fixture integration passes; live DeepSeek path remains unverified. Publish hash guard còn thiếu. |
...
Chi tiết theo ID và tiêu chí ở [task-board.md](task-board.md). AI revise code path đã có API/worker fixture test và mock browser coverage; còn provision DeepSeek key + live model/generation/revise smoke → isolated PostgreSQL/Compose runtime → real-provider browser E2E → Meta permissions → hardening. Manual metrics import, report, feedback, brief revision và outcome comparison đã có fixture/API path; không coi đó là Meta sync, live provider test hoặc production runtime.
