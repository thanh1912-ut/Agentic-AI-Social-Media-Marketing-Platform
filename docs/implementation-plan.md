...
- MEDIA-001 now has a tenant-scoped image asset API and editor flow for JPEG/PNG/WebP, bounded upload validation, authenticated preview/download, versioned attach/detach, approval content hash, and CSV/XLSX media reference. Backend integration test, 14 desktop/mobile campaign E2E tests, frontend build and PostgreSQL/SQLite migration 0001→0009 pass. Meta image publishing remains external and unimplemented.
...
- Python 3.11.16 và Node 26.7.0 có sẵn. Redis local trả `PONG`; Docker/Podman và MinIO không khả dụng. Đã tạo PostgreSQL 18.3 + pgvector 0.8.2 disposable cluster riêng, chạy migration 0001→0009 và rerun; cluster được dừng sau test, PostgreSQL 15 trên máy không bị thay đổi. Python và npm dependencies đã được cài trong checkout riêng.
...
| 1. Môi trường chạy | Python/Node deps; PostgreSQL/Redis; object storage hoặc local test adapter | Cài từ lock/manifest; migration 0001→0009 trên PostgreSQL/pgvector cô lập đã pass; còn nghiệm thu API, worker, scheduler, Redis/MinIO và health/readiness trên full runtime. Không dùng DB production. |
