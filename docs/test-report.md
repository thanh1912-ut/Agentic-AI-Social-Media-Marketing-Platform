...
Cập nhật: 2026-09-24 06:57 (Asia/Ho_Chi_Minh). Snapshot code `5a09c76` trên branch `codex/product-v1-completion` bổ sung abstention theo semantic margin và lexical stopwords cho local FastEmbed multilingual E5. Full Python suite hiện tại: **116 passed, 1 skipped**; skip là smoke thật DeepSeek vì chưa có key. Kết quả dưới đây phân biệt adapter test, synthetic retrieval evaluation, migration test và live provider test.
...
| Full Python suite; OpenAPI check; `compileall`; Compose YAML; `git diff --check` | **116 passed, 1 skipped; PASS** cho các kiểm tra còn lại | Live DeepSeek smoke là skip duy nhất. Docker Compose runtime vẫn chưa chạy vì máy thiếu Docker/Podman. |
