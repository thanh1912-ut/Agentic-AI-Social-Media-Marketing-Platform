...
Cập nhật gần nhất: 2026-09-24 06:57 (Asia/Ho_Chi_Minh)

## Trạng thái hiện tại

- Branch: `codex/product-v1-completion`; code snapshot `5a09c76` đã có trên PR #1. Bản cập nhật này bổ sung relevance abstention và evidence chạy adapter E5 thật.
...
- Mốc RAG mới: local FastEmbed multilingual E5 small pinned 384d đã chạy qua adapter; gate yêu cầu semantic margin hoặc lexical evidence mạnh. PostgreSQL migration legacy `vector(1536)` giữ dữ liệu cũ, nhận vector 384 và lọc theo model identity. Full Python suite hiện **116 passed, 1 skipped**.
...
| Security/operations | CSRF, tenant checks, rate limits, upload validation, approval hash | 116 pytest passed/1 live-DeepSeek skip; PostgreSQL pg_dump/restore counts khớp; local object archive checksum roundtrip 2 objects | `docs/security-review.md`, `docs/test-report.md` | Worker lease/restart, Redis/MinIO, proxy/edge, scheduled backup, load smoke và full operational review còn mở. |
...
- Latest retrieval hardening on code snapshot `5a09c76`: **30 focused tests**, full backend **116 passed, 1 skipped**; local model run `scripts/evaluate_retrieval.py --cache-dir /private/tmp/agentic-e5-provider-check --local-files-only` — **6/6 positive Hit@1, 0/8 no-answer false contexts**. Python compileall and `git diff --check` pass. The live DeepSeek smoke remains the only pytest skip.
