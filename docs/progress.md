...
Cập nhật gần nhất: 2026-09-23 22:00 (Asia/Ho_Chi_Minh)
...
- Phase 0 (audit và sáu tài liệu) đã hoàn tất. Phase 1 có Python/npm dependencies; clean SQLite migration 0001→0004 pass. Chưa nghiệm thu stack PostgreSQL/Redis/MinIO bằng Docker.
...
| Metrics/dashboard | Chưa có API/import/snapshot/dashboard production path | Chưa nghiệm thu | Hiện analytics helper/contract không đủ để claim có metrics thật. |
| Recommendation | Chưa có evidence-backed API/apply path | Chưa nghiệm thu | Cần metrics có nguồn trước. |
| Frontend | Auth/workspace/docs/brand/campaign/manual post/export UI và API client | Typecheck, 39 unit tests, lint, production build; mock Playwright E2E 28/28 pass | Real API E2E chưa chạy do thiếu test account/API environment. |
...
- Python: `80 passed, 1 skipped`; test mới xác nhận production worker fail-closed trước provider khi chưa có approval. Skip là live DeepSeek smoke.
- Frontend: `39 passed`; typecheck, lint, production build pass.
- Playwright mock E2E: `28 passed` across desktop and mobile Chromium; this tests MSW/demo data, not the live API.
...
3. Xây tiếp metrics import/dashboard/recommendation và publish guard trước khi gọi sản phẩm pilot hoàn chỉnh.
